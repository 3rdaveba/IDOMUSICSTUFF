      ...
      <div className="content-container">
        ...
        <div className="mb-10">
          <span className="text-eyebrow block mb-3" style={{ color: 'var(--text-tertiary)' }}>
            {t('discography.eyebrow')}
          </span>
          <ParallaxHeader
            line1={t('discography.line1', { streams: streamingStats.totalSpotifyStreams })}
            line2={t('discography.line2')}
            triggerSelector="#discography"
          />
          <p className="mt-6 text-base md:text-lg font-light max-w-2xl" style={{ color: 'var(--text-secondary)' }}>
            {t('discography.intro', { tracks: streamingStats.tracksWithData, source: streamingStats.dataSource })}
          </p>
        </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>
              {t('discography.spotify', { streams: streamingStats.totalSpotifyStreams })}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Youtube size={13} style={{ color: 'var(--accent-amber)' }} />
            <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>
              {t('discography.youtube', { views: (streamingStats.totalYoutubeViews / 1000).toFixed(1) })}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <TrendingUp size={13} style={{ color: 'var(--accent-amber)' }} />
            <span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>
              {t('discography.tiktok', { views: (streamingStats.totalTikTokViews / 1000).toFixed(0) })}
            </span>
          </div>
        </div>
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    {entry.milestones && entry.milestones.length > 0 && (
                      ...
                      >
                        <Award size={9} /> {t('discography.charted')}
                      </span>
                    )}
                  </div>
        ...
        <p className="text-[10px] mt-5" style={{ color: 'var(--text-tertiary)' }}>
          {t('discography.disclaimer', { date: streamingStats.lastUpdated })}
        </p>
      </div>
    </section>
  )
